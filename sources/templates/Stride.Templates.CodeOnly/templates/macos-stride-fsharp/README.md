...
This project consumes pinned Stride engine packages from GitHub Packages. The
generated `NuGet.config` already includes the feed URL, but GitHub Packages
still needs a one-time authenticated source entry on each machine:
